...
/*
 * TKOS - TKX Yukleyici Implementasyonu
 * Detayli format aciklamasi icin exec.h basina bakiniz.
 */
...
int exec_load(fat16_volume_t *vol, const char *filename, task_t **out_task) {
    ...
    /* Dosyayi tumuyle heap'e oku (gecici arabellek - image'a
     * kopyalandiktan sonra hemen serbest birakilir). */
    ...
    /* Kod/veri icin sayfa-hizali, SIFIRLANMIS bellek ayir (bss'siz
     * global degiskenler dogru baslangic degeriyle - 0 - baslasin). */
    ...
    /* Yuklenen imaji gorevle iliskilendir: gorev sonlandiginda
     * (sched.c temizligi) bu bellek de geri verilsin - aksi halde
     * her exec() bir sizinti (leak) olur. */
