...
/*
 * TKOS - Marquee Implementasyonu
 * font_get() ile piksel bazli glyph cizimi - fb_console.c'nin
 * render_char()'iyla ayni mantik, sadece kolon/satir yerine
 * serbest piksel x konumunda ciziyoruz.
 */
...
/* Marquee kutusunun disina tasan pikselleri kesen (clip) yazma islemi -
 * karakterler kutunun sag/sol kenarindan yarim girip yarim cikacagi icin
 * bu clip olmadan komsu widget'lara tasma olur. */
...
static void marquee_draw(marquee_t *m) {
    /* Once tum kutuyu arka plan rengiyle temizle */
    fb_fill_rect((uint32_t)m->x, (uint32_t)m->y, (uint32_t)m->w, (uint32_t)m->h, m->bg);

    /* offset arttikca metin sola kayar - sagdan girip soldan cikan
     * klasik ticker yonu. start_x, metnin ilk karakterinin (kirpilmamis)
     * teorik piksel konumu. */
    ...
    for (int32_t i = 0; i < len; i++) {
        ...
        if (cx + FONT_W <= m->x || cx >= m->x + m->w) continue; /* tamamen disarida */
...
void marquee_tick(void) {
    for (int i = 0; i < MARQUEE_MAX_COUNT; i++) {
        ...
        m->offset++;
        /* Metin + kutu genisligi kadar kaydiktan sonra basa don:
         * yani metin tamamen soldan ciktiktan sonra tekrar sagdan girer. */
