...
/*
 * TKOS - FAT16 Dosya Sistemi Surucusu (Implementasyon)
 *
 * Disk duzeni (bkz. fat16.h FAT16_PARTITION_LBA):
 *
 *   [Boot Sector]      1 sektor  - BPB burada
 *   [Reserved]         (BPB.reserved_sector_count - 1 sektor daha)
 *   [FAT #1]           BPB.sectors_per_fat sektor
 *   [FAT #2]           BPB.sectors_per_fat sektor (num_fats genelde 2)
 *   [Root Directory]   (root_entries * 32 byte) / bytes_per_sector sektor
 *   [Data Area]        cluster 2'den itibaren baslar
 *
 * Cluster numaralari 2'den baslar (0 ve 1 rezerve). Data alaninda
 * N. cluster'in LBA'si:
 *   data_start_lba + (N - 2) * sectors_per_cluster
 *
 * COKLU DISK: Her fonksiyon bir fat16_volume_t* alir, boylece ayni
 * anda birden fazla mount edilmis disk (master + slave) bagimsiz
 * olarak kullanilabilir. Global/static tutulan tek sey, tum
 * volume'ler arasinda paylasilan gecici sektor arabellegidir
 * (g_sector_buf) - bu, tek seferde tek bir I/O islemi yapildigi
 * icin (cagrilar birbirini kesmiyor, tek cekirdek + shell'den
 * senkron cagri modeli) guvenlidir.
 */

/* ------------------------------------------------
 * Ham disk yapilari (packed, disk formatiyla BIREBIR ayni)
 * ------------------------------------------------ */
...
typedef struct __attribute__((packed)) {
    ...
    /* FAT16 uzatilmis blok (BPB'den sonra) - su an kullanmiyoruz
     * ama referans icin dursun: drive_number, boot_signature,
     * volume_id, volume_label[11], fs_type[8] */
} fat16_bpb_t;

typedef struct __attribute__((packed)) {
    uint8_t  name[8];       /* Bosluklarla doldurulmus, uzanti haric */
    uint8_t  ext[3];        /* Bosluklarla doldurulmus uzanti */
    uint8_t  attr;
    uint8_t  reserved;
    ...
    uint16_t first_cluster_hi; /* FAT16'da 0, FAT32'de kullanilir */
    uint16_t write_time;
    uint16_t write_date;
...
#define FAT16_CLUSTER_EOC_MIN   0xFFF8  /* Bu ve uzeri: zincir sonu */

/* Tek sektorluk genel amacli arabellek (FAT girisi/dizin taramasi icin).
 * Volume'ler arasinda paylasilir - bkz. yukaridaki dosya basi yorumu. */
...
/* "DOSYAADI" + "UZN" (8.3, boslukla dolgulu) -> "DOSYAADI.UZN" (normal, '\0' sonlu) */
...
int fat16_init(uint8_t drive, fat16_volume_t *vol) {
    ...
    /* Once diskin gercekten var olup olmadigini hizlica kontrol et.
     * Ozellikle ikinci disk (slave) bagli degilse, dogrudan
     * ata_read_sectors timeout'una (ATA_POLL_TIMEOUT) takilmadan
     * once burada erken donebiliyoruz - ayni maliyeti tasisak da
     * (ata_drive_present da bir okuma dener) niyet acisindan net. */
    if (!ata_drive_present(drive)) return -1;

    /* Boot sektorunu oku (LBA: FAT16_PARTITION_LBA) */
    ...
    /* Cok temel bir gecerlilik kontrolu */
    ...
    /* YAZMA DESTEGI: total_clusters hesabi - bos cluster taramasinin
     * nerede duracagini bilmemiz gerekiyor. */
    uint32_t total_sectors = bpb.total_sectors_16
                              ? bpb.total_sectors_16
                              : bpb.total_sectors_32;
    if (total_sectors == 0) return -1; /* gecersiz BPB */
...
/* ------------------------------------------------
 * FAT tablosu okuma - cluster N'nin bir sonraki cluster'ini dondurur.
 * FAT16'da her giris 2 byte (uint16_t), sektor basina 256 giris var.
 * ------------------------------------------------ */

static int fat16_get_next_cluster(fat16_volume_t *vol, uint16_t cluster, uint16_t *out_next) {
    if (!vol || !vol->mounted) return -1;

    uint32_t fat_offset       = (uint32_t)cluster * 2; /* byte cinsinden */
...
int fat16_list_root(fat16_volume_t *vol, fat16_dirent_t *out_entries, int max_entries) {
    ...
    for (uint32_t s = 0; s < vol->root_sector_count && found < max_entries; s++) {
        ...
        for (int i = 0; i < entries_per_sector && found < max_entries; i++) {
            ...
            if (first_byte == 0x00) return found; /* Dizin bitti */
            if (first_byte == 0xE5) continue;      /* Silinmis giris */
...
int fat16_find_in_root(fat16_volume_t *vol, const char *name, fat16_dirent_t *out) {
    ...
    for (uint32_t s = 0; s < vol->root_sector_count; s++) {
        ...
        for (int i = 0; i < entries_per_sector; i++) {
            ...
            if (first_byte == 0x00) return 0; /* Dizin bitti, bulunamadi */
...
int fat16_read_file(fat16_volume_t *vol, const fat16_dirent_t *entry,
    ...
    if (entry->attr & FAT_ATTR_DIRECTORY) return -1; /* Dizin okunamaz */
    ...
    /* Cluster 0: dosya bos (0 byte) - normal durum, hata degil */
...
#define FAT16_CLUSTER_EOC    0xFFFF   /* zincir sonu isareti (yazarken kullanilan deger) */

/* ------------------------------------------------
 * fat16_string_to_name()
 * "DOSYA.TXT" (normal, '\0' sonlu) -> 8 byte isim + 3 byte uzanti
 * (bosluk dolgulu, BUYUK harf). fat16_name_to_string()'in tersi.
 *
 * Donus: 1 basari, 0 gecersiz isim (isim/uzanti parcasi cok uzun,
 * ya da isim boluma bos).
 * ------------------------------------------------ */
static int fat16_string_to_name(const char *filename, uint8_t *name8, uint8_t *ext3) {
    ...
    while (*p && *p != '.') {
        if (ni >= 8) return 0; /* isim parcasi 8 karakteri asiyor */
        name8[ni++] = (uint8_t)fat16_upper(*p);
        p++;
    }
    if (ni == 0) return 0; /* ".TXT" gibi bos isim gecersiz */

    if (*p == '.') {
        p++;
        int ei = 0;
        while (*p) {
            if (ei >= 3) return 0; /* uzanti 3 karakteri asiyor */
...
/* ------------------------------------------------
 * fat16_set_fat_entry()
 * Bir cluster'in FAT girisini yazar. TUM FAT kopyalarina
 * (num_fats genelde 2) ayni degeri yazar - iki kopyanin
 * birbirinden sapmasini onlemek icin.
 * ------------------------------------------------ */
 ...
 * Bos (0x0000) ilk cluster'i bulur. alloc.c'deki find_free_pages()
 * ile ayni ruhta: basit dogrusal tarama, optimizasyon yok.
 *
 * Donus: cluster numarasi (>=2), yoksa -1 (disk dolu).
 * ------------------------------------------------ */
...
/* ------------------------------------------------
 * fat16_free_chain()
 * Bir cluster zincirinin tamamini FAT'ta bos (0x0000) isaretler.
 * ------------------------------------------------ */
...
/* ------------------------------------------------
 * fat16_locate_slot()
 * Root dizininde RAW 8.3 isme (name8/ext3) gore arama yapar.
 * Bulunursa dirent'in bulundugu (mutlak LBA, sektor-ici index)
 * yazilir ve 1 doner (mevcut dosya).
 * Bulunamazsa, kullanilabilecek ILK BOS SLOT (silinmis 0xE5 ya
 * da dizin sonu 0x00) ayni sekilde yazilir ve 0 doner.
 * Hic bos slot da yoksa (root tamamen dolu) -1 doner.
 * ------------------------------------------------ */
static int fat16_locate_slot(fat16_volume_t *vol, const uint8_t *name8,
    ...
    for (uint32_t s = 0; s < vol->root_sector_count; s++) {
        ...
        for (int i = 0; i < entries_per_sector; i++) {
            ...
            if (first_byte == 0x00) {
                /* Dizin burada bitiyor - ne varolan dosya ne de
                 * daha ileride bir giris var demektir. */
                if (!free_found) {
    ...
    return -1; /* root dizini tamamen dolu, bos slot yok */
}
...
int fat16_write_file(fat16_volume_t *vol, const char *filename,
    ...
    if (found < 0) return -1; /* root dizini dolu */

    /* Mevcut dosyaysa: eski cluster zincirini once serbest birak.
     * Boylece ayni dosyayi ayni/farkli boyutla yeniden yazmak,
     * kendi eski alanini da yeniden kullanabilir. */
    if (found == 1) {
    ...
    /* Yeni veri icin cluster zinciri tahsis et */
    ...
    for (uint32_t i = 0; i < n_clusters; i++) {
        ...
        if (c < 0) {
            /* Disk dolu: simdiye kadar tahsis edileni geri al */
    ...
    /* Veriyi tahsis edilen cluster'lara yaz */
    ...
    while (cluster != 0 && bytes_written < size) {
        ...
        for (uint32_t s = 0; s < vol->sectors_per_cluster && bytes_written < size; s++) {
            ...
            if (ata_write_sectors(vol->drive, cluster_lba + s, 1, g_sector_buf) != 0)
                return -1; /* NOT: buraya kadar tahsis edilen clusterlar kalir (best-effort) */
    ...
    /* Dizin girisini yaz: isim, oznitelik, boyut, ilk cluster */
    if (ata_read_sectors(vol->drive, dirent_sector, 1, g_sector_buf) != 0) return -1;
    fat_raw_dirent_t *raw = (fat_raw_dirent_t *)g_sector_buf;
...
int fat16_delete_file(fat16_volume_t *vol, const char *filename) {
    ...
    int      dirent_index;
    int found = fat16_locate_slot(vol, name8, ext3, &dirent_sector, &dirent_index);
    if (found != 1) return -1; /* dosya yok */

    if (ata_read_sectors(vol->drive, dirent_sector, 1, g_sector_buf) != 0) return -1;
    fat_raw_dirent_t *raw = (fat_raw_dirent_t *)g_sector_buf;
    fat_raw_dirent_t *e   = &raw[dirent_index];

    if (e->attr & FAT_ATTR_DIRECTORY) return -1; /* dizin silme desteklenmiyor */

    uint16_t cluster = e->first_cluster_lo;
    if (cluster != 0) fat16_free_chain(vol, cluster);

    e->name[0] = 0xE5; /* silinmis isareti */
