...
/*
 * TKOS - Buton Widget Implementasyonu
 * Klasik 3D "kabarik/basik" buton cercevesi - her masaustu OS'un
 * standart buton gorunumunun temel mantigi budur.
 */

#define COL_FACE  7   /* acik gri - buton yuzeyi          */
#define COL_LIGHT 15  /* beyaz    - kabarikken ust/sol     */
#define COL_DARK  8   /* koyu gri - kabarikken alt/sag      */
#define COL_TEXT  0   /* siyah yazi                        */

#define FONT_W 8
#define FONT_H 8

static button_t buttons[BUTTON_MAX_COUNT];
static int      pressed_id = -1;  /* su an basili tutulan buton, yoksa -1 */
...
/* Kabarik (raised) / basik (sunken) 3D cerceve */
static void draw_bevel(const button_t *b, int is_pressed) {
    ...
    fb_fill_rect((uint32_t)b->x, (uint32_t)b->y, (uint32_t)b->w, 1, top_left);              /* ust  */
    fb_fill_rect((uint32_t)b->x, (uint32_t)b->y, 1, (uint32_t)b->h, top_left);              /* sol  */
    fb_fill_rect((uint32_t)b->x, (uint32_t)(b->y + b->h - 1), (uint32_t)b->w, 1, bot_right); /* alt  */
    fb_fill_rect((uint32_t)(b->x + b->w - 1), (uint32_t)b->y, 1, (uint32_t)b->h, bot_right); /* sag  */
}
...
void button_handle_mouse(int32_t x, int32_t y, uint8_t buttons_state) {
    ...
    if (left_now && !left_prev) {
        /* Yeni basma anı: mouse hangi butonun uzerindeyse "basili" yap */
        for (int i = 0; i < BUTTON_MAX_COUNT; i++) {
        ...
        /* Birakma ani: sadece basilan buton hala mouse'un altindaysa
         * tiklama sayilir (baska yerde birakilirsa - surukleyip
         * vazgecme - saymaz, her OS'ta boyledir). */
